...
この章では、学習の入口として、ローカルで起動している現在のFlaskアプリにPlaywrightからアクセスします。第5章で扱ったテスト用DB分離は、テストを安定させるための重要な方針ですが、ここではまず「ブラウザ操作を自動化して結果を確認する」ことに集中します。
...
この章のテストは、アプリのDBを直接初期化しません。トップページの表示だけを確認するため、DB状態に依存しにくい最小のテストから始めます。
